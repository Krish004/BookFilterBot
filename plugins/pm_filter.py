...
async def give_filter(client, message):
    try:
        ...
        pass
    # 🔒 Block Links in Groups (skip admins)
    if re.search(LINK_REGEX, message.text):
        if not await admin_check(message):
            try:
                await message.reply_sticker(
                    "CAACAgUAAxkBAAEDu3FlYwV7VfGyoXKaThFZcXk3aMcMBgACVQIAAnlc4FfP4cGZpqJ_HDQE"
                )
                await message.reply_text("🚫 Don't post links here!")
            except Exception:
                pass
            try:
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = prepare_query(message.text)   # ✅ cleaned query for books
    ...
    # ✅ Buttons (keep as is – no remove)
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') 
                        and not x.startswith('@') 
                        and not x.startswith('www.'), file.file_name.split()))}", 
